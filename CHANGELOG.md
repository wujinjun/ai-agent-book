...
## v2026.8.3 - 2026-08-15

- 完成全书内容、结构与视觉连续性终审：统一 38 章的总结、练习、面试题、延伸阅读和章节过渡，补齐版本证据与正式引用，并连续复核十个项目的难度递进。
- 正文最终采用 254 张 Mermaid 工程图与 57 张出版信息图；13 张工程图联系表和 5 张信息图联系表完成全量检查，工程图宽高比超过 6:1 的数量降为 0。
- 重建 MkDocs Material HTML 阅读版，移除维护与培训运营页面，修复目录型章节路由；全站断链、缺失资源和残留 Mermaid 围栏审计通过。
- 重建 685 页 A4 PDF，统一封面、目录、篇章页、正文、代码、表格、图题、页眉和页码；使用 Poppler 对封面、正文、工程图、信息图、项目和索引代表页完成栅格化复核。
- 将 EPUB3 优化至约 52 MiB，正式容器包含 67 个 XHTML、58 个 PNG 和 251 个 SVG；修复全书索引 787 个跨章断链，XML、资源与片段引用审计通过。
- 归档完整审查报告、整改计划和 HTML/PDF/EPUB 专项 QA；明确区分仓库内编辑完成与实体设备、印刷打样及专业权利审查边界。

## v2026.8.2 - 2026-08-13

- 完成当前源码全书图示终审：260 张 Mermaid 工程图与 48 张出版级信息图分别通过 13 页和 4 页联系表复审；新增工程图源文件、SVG、PNG 和 manifest 记录全部落盘。
- 重建 MkDocs HTML、558 页 A4 PDF 与 EPUB3；修复打印源信息图路径和 Chrome 完成写入后挂起问题，PDF 改用可解析文件稳定检测与原子发布。
- 固定代码、语法高亮与数学文本的出版字体；将第 16 章检索验收公式改为跨 HTML/PDF/EPUB/终端稳定的 ASCII 记法，发行预检不再发现 Menlo 等未批准系统字体。
- 候选版本没有同名 Changelog 小节时，发行说明自动回退到 `Unreleased` 内容，避免生成空泛占位说明。
- 离线 HTML 压缩包保留站内 PDF/EPUB 下载入口，但不再在压缩包根目录重复嵌入同一大文件；独立附件保持不变，显著降低 Release 上传体积。
- 当前 Python 3.12 根级套件增至 291 项；Ruff、严格 MkDocs、编辑、隐私、仓库卫生、图示和三格式出版审计通过。
...
- 完成出版信息图升级：A 级核心图 12/12、B 级增强图 20/20、全章与十项目覆盖图 16/16；该阶段保留 240 张 Mermaid，当前候选已扩展并重建为 260 张。
...
- 阶段性重建并验收 MkDocs HTML、535 页 A4 PDF 与 EPUB3；Ruff、272 项测试、严格文档构建、出版和发行资产审计通过。该数字是 2026-08-12 中间候选快照，当前候选见本节顶部。
