...
仓库状态审计日期：2026-08-15。外部框架与规范的实际核对日期单独记录在 `notes/version-check.md`；仓库审计日期不能替代 API 版本核查日期。
...
当前版本是一套**完成仓库内编辑终审的多格式出版候选版**。38 章均有连续正文、图示、代码块、练习、面试题、延伸阅读和统一导航；十个项目形成从最小助手到企业平台的递进式教学主线。HTML、685 页 PDF 与 EPUB3 已从同一正文源重建并通过出版审计，维护与培训运营页面不再混入正式读者版。

“出版候选版”不等于出版社签字终稿，也不等于十个项目均可直接投入生产。实体设备、印刷打样和专业权利审查未由本轮执行；按用户要求，这些外部事项不作为仓库内内容与版式整改的阻塞条件。
...
- 全书当前状态为仓库内 `editor_reviewed` 出版候选稿；审查记录与整改结果见 `notes/2026-08-15-book-content-layout-visual-review.md`、`notes/2026-08-15-book-remediation-plan.md`。
- 结构门禁、全量图稿联系表和格式审计与人工内容/样张复核配合使用；任何单一自动指标都不单独代表出版质量。
...
- 当前正文使用 254 张 Mermaid 工程图与 57 张出版信息图。工程图最大宽高比 5.90，超过 6:1 的图为 0；13 张工程图联系表和 5 张信息图联系表已全量复核。HTML 使用 SVG/高分辨率 PNG，PDF 使用 SVG/印刷 PNG，EPUB 使用 SVG/轻量 PNG，三种格式都不依赖阅读器现场执行 Mermaid。
- HTML 阅读版通过 MkDocs 严格构建和出版审计，断链、缺失资源及残留 Mermaid 围栏为 0；维护、验收与培训运营页面已从正式阅读站点移出，下载目录包含同批 PDF 与 EPUB。
- PDF 于 2026-08-15 从现行源码重建：A4 共 685 页、56,659,002 字节，SHA-256 为 `91baa292d56023b94400ef8fc5c11a20ccd50da6f25da699cec93134bfd52207`。封面、目录、正文、信息图、工程图、项目章首、技术选型章和索引代表页已用 Poppler 栅格化复核。
- EPUB3 为 54,319,199 字节，SHA-256 为 `0726996659bbd75ea6336e94475ff09e7fbca0b24ac3dc5af4c6e600d2420f83`；含 67 个 XHTML、58 个 PNG、251 个 SVG 和 382 个压缩成员。全书索引的 787 个跨章断链已修复为 0，容器、资源、XML 和片段引用审计通过。
- 正式 PDF/EPUB 均以全书索引收束，培训材料继续作为仓库独立资源保存，不再进入读者书稿。格式专项验收分别见 `notes/html-responsive-qa.md`、`notes/pdf-visual-qa.md` 和 `notes/epub-device-qa.md`。
- 本轮产物以 `v2026.8.3` 发布；Release 附件包含离线 HTML、685 页 PDF、EPUB3、培训 PPTX、发行说明、清单和 SHA-256 校验文件。
...
2026-08-15 当前批次完成 254 张工程图与 57 张正文信息图复核，重建 685 页 A4 PDF、EPUB3 与离线 HTML，并通过三格式出版审计。13 张工程图联系表与 5 张信息图联系表完成全量目视复审；PDF 最终样张覆盖封面、目录、正文、矢量工程图、栅格信息图、项目、索引密集表和末页。仓库内内容与版式整改已经完成，实体设备、印刷打样及专业权利审查不在本轮能力范围内，也不作为当前完成状态的阻塞项。
