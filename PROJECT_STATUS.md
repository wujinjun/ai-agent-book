...
- 当前 Python 3.12.13 环境下，283 项根级与项目测试全量通过。其中十项目共享服务契约按项目参数化验证持久化、幂等、租户权限、预算、取消、SSE 回放、指标与重建恢复，项目 4、8、10 另有生产恢复路径测试；独立示例、框架候选、发行资产、培训课件字体与范围化发布契约测试也纳入同一根测试集合。
...
- 项目 8 已将外部报告发布移出可重放图节点，新增持久 Publication Outbox、报告哈希、稳定幂等键、原子领取、远端回执与崩溃恢复；模拟“远端已成功、本地回执未提交”后以同一键重试只产生一个远端结果，篡改报告会在 Provider 调用前拒绝。该保证依赖真实 Publisher 支持幂等或远端查询；否则未知结果仍需人工对账。10 项项目 8 聚焦测试、Ruff 与 strict mypy 已通过，待本批结束后重跑全量测试与三格式出版。
