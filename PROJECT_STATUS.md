...
- 当前 Python 3.12.13 环境下，281 项根级与项目测试全量通过。其中十项目共享服务契约按项目参数化验证持久化、幂等、租户权限、预算、取消、SSE 回放、指标与重建恢复，项目 4、8、10 另有生产恢复路径测试；独立示例、框架候选、发行资产、培训课件字体与范围化发布契约测试也纳入同一根测试集合。
