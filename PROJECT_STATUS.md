...
- 当前 Python 3.12.13 环境下，根级与项目测试共 237 项通过；其中十项目共享服务契约按项目参数化验证持久化、幂等、租户权限、预算、取消、SSE 回放、指标与重建恢复，项目 4、8、10 另有生产恢复路径测试；独立示例及框架候选隔离测试也均通过。
...
2026-08-08 在 Python 3.12.13 下重新运行 Ruff、mypy 和 237 项根级/项目测试，全部通过。235 组 Mermaid 图、真实 Trace 截图、HTML、447 页 PDF 和 EPUB3 已重新构建；出版审计、跨 XHTML 链接检查、Apple Books、Calibre 9.13.0 与三种响应式视口复验通过。P8 另通过根 README/图片断链、密钥、仓库卫生与 Release 打包校验，全新虚拟环境完成离线 Tool Loop、5 项聚焦测试与 HTML 构建。结构化证据见 `notes/p7-layout-acceptance.yml` 和 `notes/p8-github-acceptance.yml`。公开 v2026.8.0 Release 仍是上一版 400 页出版物，当前候选尚未在 P9 正式发布，不能把本地构建冒充公开 Release。
