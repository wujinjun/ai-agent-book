...
官方资料最后核对：各条目单独记录。仓库完成度审计：2026-08-13。
...
| SQLAlchemy / Psycopg / Redis | 已安装实测基础路径；P9 历史候选完成 Docker 服务集成，当前新增恢复机制通过直接与根级测试 | SQLAlchemy 2.0.51、Psycopg 3.3.4、Redis client 8.0.1；历史容器证据见 `notes/p9-project-runtime-qa.md`。租约心跳、运行中取消与 DLQ 批准机制已在离线数据库/服务测试中验证，但尚未用当前提交重建十套容器，不外推真实集群行为 |
