...
![行情新闻公告财报绑定统一数据时点并校验时间时区缺失和质量，确定性指标行业事实与模型分析分层处理，报告区分带来源事实模型推断风险与未知，并标明生成时间截止时间引用和非投资建议](../../docs/assets/infographics/png/project07-stock-research-agent-infographic-2x.png)

*图 P7-A　股票研究的时点、来源、事实与推断边界。*

报告首先是可追溯的数据产品，其次才是语言模型分析。行情时间、新闻发布时间和财报期次必须统一解释；任何模型推断都不得伪装成事实，也不直接输出确定性买卖建议。本项目不构成投资建议。
