...
模型规模、训练计算与能力的研究关系可参见 [Scaling Laws](../references.md#ref-kaplan2020)和 [Chinchilla](../references.md#ref-hoffmann2022)，但它们不能直接推出某个线上任务的最佳模型路由。生产优化仍应以本章定义的任务成功率、尾延迟和单位成功成本实测。
...
flowchart TB
    Request["请求开始"] --> Queue["队列等待"]
    Queue --> Prefill["模型 Prefill"]
    Prefill --> Decode["Token Decode"]
    Decode --> Tools["工具调用与网络等待"]
    Tools --> Review["Reviewer / 后处理"]
    Review --> Result["首个可用结果或任务终态"]
    Retry["重试与返工"] -.放大关键路径.-> Prefill
    Retry -.放大关键路径.-> Tools
```
...
成本与性能优化必须在任务成功、安全和可恢复性约束下进行。模型路由、缓存、压缩、Batch 和并行都可能改变语义；超时应从端到端 Deadline 分配，而不是各层独立设置；降级必须明确牺牲什么能力。容量实验同时报告成功率、尾延迟、单位成功成本和错误分布。下一篇将进一步讨论多 Agent、Coding Agent、Browser Agent 和多模态系统。

## 课后练习

### 设计题

1. 为研究 Agent 制定共享预算，包含墙钟、模型调用、工具调用、输入/输出 Token 和费用，并设计 70%/90% 阈值下的动作。
2. 设计模型路由：先按能力和数据边界筛选，再按质量、P95 与单位成功成本决策；输出路由桶及各桶评估指标。
3. 设计三级降级路径，明确每一级牺牲什么，并列出永不降级的权限、审批、事实和审计门禁。

### 概念题

4. 说明并行 Tool Call 在哪些共享资源与依赖条件下反而提高尾延迟或破坏顺序语义。

### 编码题

5. 为缓存键加入 Tenant、Permission Version、Model、Prompt、Data Version 和输入哈希。输入为撤权前后同一查询；输出必须出现不同 Key；检查标准是旧权限结果不可再命中。

### 故障实验

制造模型层、Tool 层和 Job 层各自重试三次的嵌套配置，计算最坏尝试数，再改为共享重试预算并比较调用量。
...
1. 如何计算每个成功任务的单位成本？
2. 并行工具在什么情况下反而增加尾延迟？
3. 缓存键为什么需要包含租户、权限和数据版本？

## 延伸阅读与代码目录

延伸阅读包括目标模型 Usage 与价格文档、OpenTelemetry、缓存一致性和性能测试资料。项目8与10提供共享预算、路由、降级和平台级成本归因案例。
