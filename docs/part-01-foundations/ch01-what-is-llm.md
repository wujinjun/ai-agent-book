...
本节对 Transformer 历史位置、规模化语言建模、指令对齐和“随机鹦鹉”争议的讨论，分别以 [Transformer 原始论文](../references.md#ref-vaswani2017)、[规模化语言模型研究](../references.md#ref-brown2020)、[指令对齐研究](../references.md#ref-ouyang2022)和 [Bender 等人的批判性分析](../references.md#ref-bender2021)为主要依据；这些资料支持不同层面的论断，不能互相替代。
...
输入至少包含合法工具、未知工具和非法参数三类 Fixture；输出为类型化成功或错误；检查标准是测试全程离线且任何非法调用都不会进入执行函数。
