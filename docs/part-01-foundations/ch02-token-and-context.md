...
子词切分的工程背景可参见 [BPE 研究](../references.md#ref-sennrich2016)与 [SentencePiece](../references.md#ref-kudo2018)；上下文变长不保证模型等量利用所有位置，相关实验见 [Lost in the Middle](../references.md#ref-liu2023lostmiddle)。因此，本章把“窗口容量”和“信息可用性”作为两个不同指标。

下面的信息图先建立本章的预算心智模型：上下文窗口是一只有硬容量上限的工作容器，系统规则、当前问题和输出预留属于优先保证的区域，历史、检索证据与工具结果则竞争弹性空间。超限时应按信息价值选择保留、截断、摘要或外部检索，而不是机械删除最早消息。

![上下文窗口在系统规则、当前问题、工具定义、历史、检索证据和输出预留之间分配有限预算，超限后采用保留、截断、摘要或外部检索，并与长期持久存储区分](../assets/infographics/png/context-budget-memory-infographic-2x.png)

*图 2-A：上下文预算、压缩策略与长期记忆边界。容量分区表达优先级和职责，不表示所有请求必须使用固定百分比。*

图 2-A 最底部把外部持久存储单独画出，因为会话结束后，上下文内容不会自动成为长期记忆。只有经过明确写入策略、权限和保留期治理的信息，才能在未来请求中被检索并重新装配进上下文。
...
Token 是模型计算单位，上下文是单次调用的工作区，不是持久记忆。窗口管理同时影响质量、成本、延迟和安全。下一章将进入 Transformer 内部，解释 Token 表示如何通过 Attention 在序列位置之间交换信息。

## 课后练习

### 概念题

解释“上下文窗口是单次调用工作区，不是长期记忆”的含义，并分别给出应保留原文、结构化事实、摘要和检索引用的例子。

### 设计题

1. 为一段 50 轮客服对话设计“固定指令 + 近期窗口 + 事实摘要 + 外部检索”策略。输出一张上下文分区表，说明每类信息的保真方式、淘汰条件和权限边界。

### 编码题

2. 为本章预算器补充边界测试。输入至少覆盖零预算、固定区超限、单条消息超长和输出预留不足；输出为可重复的测试报告；检查标准是关键约束不会被静默截断。
3. 使用目标 Tokenizer 比较语义相近的中文、英文、JSON 与 Python 代码。输出模型/Tokenizer 版本、样本文本和 Token 数；检查标准是结论来自实测而非字符数估算。

### 故障实验

4. 构造一次“摘要把两个订单号合并”的故障，解释为什么订单号不能只存在生成摘要中，并给出结构化事实记录与摘要之间的引用方案。
...
1. 上下文窗口翻倍为什么不一定使长文问答质量翻倍？
2. 滑动窗口和检索式记忆分别会丢失什么？
3. 为什么本地 Token 估算必须与服务端 Usage 持续校准？

## 延伸阅读与代码目录

延伸阅读包括目标模型官方 Tokenizer 文档、Sennrich 等人的 BPE 研究与 Liu 等人的 *Lost in the Middle*。本章代码目录为 [`examples/token_counter/`](https://github.com/wujinjun/ai-agent-book/tree/main/examples/token_counter)，已提供独立 Python 3.12 工程、离线 Fixture、预算拒绝测试与预期输出；Fixture 计数仍不得用于真实模型计费。
