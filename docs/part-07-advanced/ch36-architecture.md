...
下面的信息图按入口身份、运行控制、能力服务、执行存储和治理证据五个层次组织平台。左右两侧分别强调身份与策略边界、数据与外部能力边界；中间主路径则说明一次 Run 如何从入口进入 Runtime，再组合工作流、模型、工具、检索和持久状态。

![企业 Agent 平台从身份入口、运行控制、能力服务、执行存储到治理证据的分层参考架构](../assets/infographics/png/enterprise-agent-platform-infographic-2x.png)

*图 36-A：企业 Agent 平台分层参考架构。同一行卡片表达同层职责，纵向箭头表达一次运行经过的主要层次；图中的层是逻辑边界，不要求每层独立部署成微服务。*

信息图把治理证据放在基础设施之后，并不表示治理只在请求完成后发生。Policy、Security 和预算检查必须参与运行前与工具调用前的决策；Trace、Metrics、Audit 和 Evaluation 则持续记录过程并形成发布反馈。左右两侧的身份与数据边界也不是旁路组件，而是所有入口、检索和工具调用都必须遵守的约束。

下面继续给出可逐边追踪的精确组件关系图。它按控制、能力、状态、执行与治理职责划分组件，而不是按框架名称拆服务。
