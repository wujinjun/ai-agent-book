...
![MCP Server 从启动配置能力注册初始化协商，到 stdio 与远程 HTTP 传输，再到参数权限资源范围超时错误限制和优雅关闭的生命周期](../assets/infographics/png/mcp-server-lifecycle-infographic-2x.png)

*图 12-A：MCP Server 生命周期、传输与生产边界。stdio 与远程 HTTP 的认证和进程边界不同，但都必须执行服务端授权。*

图 12-A 最底部把日志、健康、取消和关闭放在协议能力之外：这些是服务器作为长期运行进程的工程职责。stdio 服务尤其不能把日志写入协议使用的标准输出流。
