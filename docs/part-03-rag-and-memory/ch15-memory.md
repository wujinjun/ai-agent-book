...
长期记忆的研究原型可参见 [Generative Agents](../references.md#ref-park2023generativeagents)和 [MemGPT](../references.md#ref-packer2023memgpt)。本章不直接照搬其角色或内存隐喻，而是把来源、同意、租户、纠错、TTL 与删除传播提升为生产数据治理要求。

## 核心原理与架构图

Memory 是带治理的数据生命周期。主图展示事件如何通过写入门禁进入存储、按任务检索，并最终被纠错或删除。

下面的信息图先把完整生命周期放在同一页。交互事件只有通过未来价值、用户同意、敏感性、来源置信和冲突检查后，才进入按租户与主体隔离的长期存储；读取时先做确定性的权限和时效过滤，再计算相关性；纠错、TTL 和删除还必须传播到索引、缓存、摘要与恢复流程。

![Agent Memory 从候选事件经过写入门禁、租户主体隔离、偏好语义事实和情景事件存储、权限时效相关性检索，到纠错过期删除传播与审计的完整生命周期](../assets/infographics/png/memory-governed-lifecycle-infographic-2x.png)

*图 15-A：Agent Memory 的受治理生命周期。纵向箭头表示记录从候选到可用记忆再到遗忘治理的主要阶段；每个门禁都应由可测试规则或用户决定执行，不能只依赖模型自评。*

图 15-A 把删除放在与写入、检索同等重要的位置。所谓“长期”不等于永久保存；如果系统无法解释记录来源、用途、过期时间和删除传播状态，它拥有的只是难以治理的历史数据，而不是可靠的 Agent Memory。
...
Memory 是受治理的数据生命周期，不是不断增长的聊天记录。候选记忆必须经过来源、用途、同意、敏感性、稳定性和冲突门禁；检索必须先做租户、主体、用途、状态与 TTL 过滤；纠错使用版本和来源而不是静默覆盖；删除必须传播到主记录、向量、缓存、摘要和备份恢复路径。下一章将讨论支撑检索式 Memory 与 RAG 的向量索引基础设施，以及它不能承担的权限和事实职责。

## 课后练习

### 设计题

1. 设计偏好记忆 Schema，覆盖租户、主体、范围、来源、置信、敏感级别、版本、状态和 TTL。

### 故障实验

2. 为全局偏好、任务级临时要求和模型推断构造冲突更新规则，明确何时替换、保留或请求确认。

### 设计题

3. 设计从逻辑删除到向量、缓存、摘要和备份恢复路径的遗忘 Saga。

### 编码题

4. 使用相同 `subject_id` 和已知 `memory_id` 设计跨租户读取、搜索、更新、删除和导出攻击测试。

输入为两个租户的同名主体；输出为全路径授权矩阵；检查标准是越权结果与受保护 Trace 均为零泄漏。

### 概念题

5. 为会话摘要、结构化事实、长期 Memory 与 Audit Log 分配不同职责和保留策略。
...
1. Memory 与上下文窗口有什么区别？
2. 哪些信息不应自动写入长期记忆？
3. 为什么审计日志不能直接作为模型自由检索的 Memory？
4. 摘要记忆与结构化事实如何分工？

## 延伸阅读与代码目录

延伸阅读包括长期对话记忆研究、隐私法规与数据生命周期治理资料。本章代码目录为 [`examples/long_term_memory/`](https://github.com/wujinjun/ai-agent-book/tree/main/examples/long_term_memory)，提供 SQLite、Fake Clock、写入门禁、来源、TTL、更正、导出、删除传播和跨租户测试。
