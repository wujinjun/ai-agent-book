...
*图 17-A：原生 Agent Runtime 的内核、端口与测试边界。框架可以替换适配层，但状态、授权、预算、终止和错误语义不应随框架漂移。*

图 17-A 的端口结构允许离线测试注入 Fake Model、Fake Tool 和内存存储，从而在没有付费 API 时验证循环、失败恢复和终止条件；真实模型只用于模型行为相关的集成评估。
