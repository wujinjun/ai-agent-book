...
模型交替推理与行动的研究背景可参见 [ReAct](../references.md#ref-yao2022)，学习使用外部工具的另一条路线可参见 [Toolformer](../references.md#ref-schick2023)。这些工作讨论模型能力；本章的授权、幂等、超时和人工审批属于宿主 Runtime 的工程责任。

主图把工具调用从“模型选择一个函数”扩展为完整的受控执行面。模型输出只是一项提议；注册表检查、参数验证、主体资源授权、风险分级、人工审批、Sandbox、幂等和审计均由 Runtime 强制执行。

![模型提出工具动作后，Runtime 从注册表取得参数 Schema，并依次检查未知工具参数主体资源授权与风险分级](../assets/infographics/png/tool-permission-execution-infographic-a-2x.png)

*图 8-A：模型提议与策略校验。模型只能提出动作，Runtime 决定工具是否存在、参数是否合法、主体是否有权。*

![只读可逆与不可逆工具进入不同审批门，执行器继续施加超时 Allowlist Sandbox 幂等状态核对步数预算终止和审计](../assets/infographics/png/tool-permission-execution-infographic-b-2x.png)

*图 8-B：风险分级与受控执行。人工审批只覆盖被展示的精确动作，不能替代超时、幂等和执行后状态核对。*

图 8-B 还特别保留“未知状态”：网络超时只说明调用方没有收到确认，不能证明外部副作用没有发生。只有在状态核对或幂等机制可用时，运行时才可以自动重试。
...
Tool Calling 的核心不是让模型“会调用函数”，而是把动作提议放入受控执行循环。工具注册表、参数 Schema、主体与资源授权、审批、超时、幂等、状态核实和审计全部由模型外运行时负责。并行由数据依赖和副作用决定；写调用超时产生的是未知状态，只有幂等键或外部状态核实能够支持安全重试。下一章将把单次工具循环扩展为具有显式状态、终止与恢复能力的 Agent Runtime。

## 课后练习

### 编码题

1. 为现有运行时增加一个幂等写工具，覆盖相同请求复用、相同键不同参数拒绝、并发竞争和超时后状态核实。

输入为固定幂等键和多组参数；输出为一次业务副作用及稳定结果；检查标准是并发与重试不产生重复写入。

### 设计题

2. 为天气、汇率、创建订单和支付订单建立依赖图，标出可以并行和必须串行的调用，并写出判断依据。
3. 设计一个稳定的工具错误契约，确保模型只能看到安全错误码和必要说明，内部堆栈保留在受保护日志中。
4. 设计绑定主体、工具名、规范化参数哈希、策略版本和过期时间的审批令牌。

### 故障实验

5. 为 Tool Loop 编写成功、未知工具、参数非法、超时、审批拒绝、状态未知、无进展和最大步数的验收清单。

### 概念题

说明“模型选择工具”“Host 校验和执行工具”“资源服务授权”分别属于哪一层，为什么不能合并。
...
1. 模型选择某个工具为什么不代表有权执行？
2. 并行工具调用在什么情况下不安全？
3. 客户端超时为什么不能直接解释为写操作失败？
4. 为什么把异常全文作为 Observation 可能造成安全问题？

## 延伸阅读与代码目录

延伸阅读包括目标供应商当前 Tool Calling 官方文档、JSON Schema 规范以及 ReAct、Toolformer 等工具使用研究。本章对应代码目录为 `examples/tool_runtime/`。运行任何真实工具前，都应先用离线 Fake 和失败注入证明上述边界能够生效。
