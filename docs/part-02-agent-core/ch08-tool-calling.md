...
![模型提出工具动作后经过注册表参数校验主体资源授权风险分级，高风险操作绑定人工审批，并由执行器施加超时沙箱幂等状态核对和审计](../assets/infographics/png/tool-permission-execution-infographic-2x.png)

*图 8-A：Tool Calling 的权限、审批、幂等与审计门禁。三条风险路径表示不同控制强度，不表示写操作可以绕过主体授权。*

图 8-A 还特别保留“未知状态”：网络超时只说明调用方没有收到确认，不能证明外部副作用没有发生。只有在状态核对或幂等机制可用时，运行时才可以自动重试。
